...
export const createCompany = async (data) => {
  return prisma.company.create({ data: sanitizeCompany(data) });
};

export const createCompanyWithLicenseAndUsers = async ({ company, license, users }) => {
  if (!company || !company.name || company.name.trim() === "") {
    const err = new Error("Les donnees de l'entreprise sont obligatoires (nom requis)");
    err.status = 400;
    throw err;
  }
  if (!license || !license.planCode || !license.startsAt) {
    const err = new Error("Les donnees de la licence sont obligatoires (planCode et startsAt requis)");
    err.status = 400;
    throw err;
  }
  if (!Array.isArray(users) || users.length === 0) {
    const err = new Error("Au moins un utilisateur doit etre fourni");
    err.status = 400;
    throw err;
  }
  ...
  for (const user of users) {
    ...
    if (seenEmails.has(normalizedEmail)) {
      const err = new Error(`Adresse email dupliquee detectee: ${normalizedEmail}`);
      err.status = 400;
      throw err;
    }
...
export const updateCompany = async (id, data, companyId) => {
  await getCompanyById(id, companyId);
